	...
	int screenWidth, int screenHeight, XMMATRIX baseViewMatrix)
{
	...
	result = m_font->Initialize(device, deviceContext, "../DirectXEngine/data/fontdata.txt", "../DirectXEngine/data/font.tga");
	if (!result)
	{
	...
	//Now update the sentence vertex buffer with the new string information
	result = updateSentence(m_sentence2, "Goodbye", 100, 100, 1.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
